...
interface ExpandableScreenTriggerProps {
  children: ReactNode
  className?: string
  // Styles the morphing background layer (the surface that flies to full
  // screen). Give it the same bg as the expanded content for a clean morph.
  bgClassName?: string
}
      ...
      {!isExpanded && (
        <motion.div className={`inline-block relative ${className}`}>
          ...
          <motion.div
            ...
            layout
            layoutId={layoutId}
            className={`absolute inset-0 transform-gpu will-change-transform ${bgClassName}`}
          />
